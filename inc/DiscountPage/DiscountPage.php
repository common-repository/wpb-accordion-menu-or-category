...
class WpBean_AccordionMenu_DiscountPage
{

	/**
	 * The shortcode builder parameters array.
	 *
	 * @var array
	 */
	public $args = array(
		...
		'parent_slug' => 'wpb_wmca_shortcodes-items',   // for submenu only.
	...
	public function admin_scripts(){
		wp_enqueue_style( 'wpb_accordion_menu_discount_page_style', plugins_url( 'assets/css/discount-page.css', __FILE__ ), '', '1.0' );
	}
						...
						if( isset($products) && '' !== $products ){
							foreach( $products as $product ){
								$permalink =  add_query_arg( array(
									...
									'utm_source' => 'AccordionMenuFreeVersion',
	...
	public function discount_admin_notice() {
		...
		$dismiss_url = wp_nonce_url(
			add_query_arg( 'wpbean-accordion-menu-discount-admin-notice-dismissed', 'true' ),
			'wpbean_accordion_menu_discount_admin_notice_dismissed',
			'wpbean_accordion_menu_discount_admin_notice_dismissed_nonce'
		);
	...
	public function discount_admin_notice_dismissed() {
		...
		if ( ! empty( $_GET['wpbean-accordion-menu-discount-admin-notice-dismissed'] ) ) { // WPCS: input var ok.
			check_admin_referer( 'wpbean_accordion_menu_discount_admin_notice_dismissed', 'wpbean_accordion_menu_discount_admin_notice_dismissed_nonce' );
			add_user_meta( $user_id, 'wpbean_accordion_menu_pro_discount_dismissed', 'true', true );
		}
	}
}
